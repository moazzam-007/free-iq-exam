(() => {
  ...
  const QUESTIONS = [
    { id: 1, part: 'A', domain: 'inattentive', shadedMin: 2, context: 'Penyelesaian tugas eksekutif', text: 'Seberapa sering Anda kesulitan menyelesaikan detail akhir suatu proyek, setelah bagian yang menantang selesai?' },
    { id: 2, part: 'A', domain: 'inattentive', shadedMin: 2, context: 'Perencanaan dan organisasi', text: 'Seberapa sering Anda kesulitan merapikan atau mengorganisasi sesuatu saat harus mengerjakan tugas yang membutuhkan keteraturan?' },
    { id: 3, part: 'A', domain: 'inattentive', shadedMin: 2, context: 'Memori prospektif (mengingat rencana)', text: 'Seberapa sering Anda bermasalah dalam mengingat janji temu atau kewajiban harian?' },
    { id: 4, part: 'A', domain: 'inattentive', shadedMin: 3, context: 'Inisiasi / memulai tugas', text: 'Saat memiliki tugas yang membutuhkan banyak pemikiran mendalam, seberapa sering Anda menghindar atau menunda untuk memulainya?' },
    { id: 5, part: 'A', domain: 'hyperactive', shadedMin: 3, context: 'Kegelisahan motorik fisik', text: 'Seberapa sering Anda menggerak-gerakkan tangan atau kaki saat harus duduk diam dalam waktu lama?' },
    { id: 6, part: 'A', domain: 'hyperactive', shadedMin: 3, context: 'Kegelisahan internal / pikiran', text: 'Seberapa sering Anda merasa terlalu aktif dan terdorong untuk terus bergerak, seolah-olah digerakkan oleh motor/mesin?' },
    { id: 7, part: 'B', domain: 'inattentive', shadedMin: 3, context: 'Perhatian terhadap detail', text: 'Seberapa sering Anda membuat kesalahan ceroboh saat harus mengerjakan tugas yang membosankan atau rumit?' },
    { id: 8, part: 'B', domain: 'inattentive', shadedMin: 3, context: 'Ketahanan fokus berkelanjutan', text: 'Seberapa sering Anda kesulitan mempertahankan fokus saat melakukan pekerjaan yang monoton atau berulang?' },
    { id: 9, part: 'B', domain: 'inattentive', shadedMin: 2, context: 'Konsentrasi mendengarkan langsung', text: 'Seberapa sering Anda kesulitan memusatkan konsentrasi pada apa yang dikatakan orang lain, bahkan saat mereka berbicara langsung dengan Anda?' },
    { id: 10, part: 'B', domain: 'inattentive', shadedMin: 3, context: 'Pengorganisasian benda harian', text: 'Seberapa sering Anda salah menaruh atau kesulitan menemukan barang-barang di rumah atau tempat kerja?' },
    { id: 11, part: 'B', domain: 'inattentive', shadedMin: 3, context: 'Keteralihan fokus (distraksi)', text: 'Seberapa sering konsentrasi Anda teralihkan oleh aktivitas atau suara di sekitar Anda?' },
    { id: 12, part: 'B', domain: 'hyperactive', shadedMin: 3, context: 'Kepatuhan duduk dalam situasi resmi', text: 'Seberapa sering Anda meninggalkan tempat duduk dalam rapat atau situasi lain yang mengharuskan Anda tetap duduk?' },
    { id: 13, part: 'B', domain: 'hyperactive', shadedMin: 3, context: 'Kegelisahan psikomotorik', text: 'Seberapa sering Anda merasa gelisah atau tidak bisa diam di dalam hati?' },
    { id: 14, part: 'B', domain: 'hyperactive', shadedMin: 3, context: 'Kemampuan relaksasi diri', text: 'Seberapa sering Anda kesulitan untuk bersantai dan rileks saat memiliki waktu luang untuk diri sendiri?' },
    { id: 15, part: 'B', domain: 'hyperactive', shadedMin: 3, context: 'Aktivitas verbal (banyak bicara)', text: 'Seberapa sering Anda mendapati diri Anda berbicara terlalu banyak dalam situasi sosial?' },
    { id: 16, part: 'B', domain: 'hyperactive', shadedMin: 2, context: 'Impulsivitas percakapan (menyela)', text: 'Saat sedang bercakap-cakap, seberapa sering Anda menyela atau menyelesaikan kalimat lawan bicara sebelum mereka selesai berbicara?' },
    { id: 17, part: 'B', domain: 'hyperactive', shadedMin: 3, context: 'Kesabaran antre dan kontrol diri', text: 'Seberapa sering Anda kesulitan menunggu giliran dalam situasi yang membutuhkan antrean atau giliran?' },
    { id: 18, part: 'B', domain: 'hyperactive', shadedMin: 3, context: 'Inhibisi perilaku terhadap orang lain', text: 'Seberapa sering Anda menginterupsi atau memotong kesibukan orang lain saat mereka sedang sibuk?' }
  ];
  ...
  function renderQuestion() {
    ...
    $('question-number').textContent = `Pertanyaan ${q.id} dari ${total}`;
    $('question-part').textContent = q.part === 'A' ? 'Bagian A · Skrining Klinis Inti' : 'Bagian B · Inventaris Gejala Tambahan';
    $('question-domain').textContent = q.domain === 'inattentive' ? 'Inatensi (Tidak Perhatian)' : 'Hiperaktivitas / Impulsivitas';
    $('question-heading').textContent = q.text;
    $('question-context').textContent = q.context;
    $('progress-fill').style.width = `${Math.max(4, progress)}%`;
    $('progress-label').textContent = `${Math.round((answeredCount() / total) * 100)}% terjawab`;
    ...
    $('next-button').textContent = state.currentIndex === total - 1 ? 'Selesaikan Kuesioner' : 'Pertanyaan Berikutnya';
    announce(`Pertanyaan ${q.id} dari ${total}. ${q.text}`);
  }
  ...
  function calculateScores() {
    ...
    let classification = 'Pola rendah / sub-klinis';
    let badge = 'Kemungkinan klinis rendah';
    let tone = 'low';

    if (partAPositive) {
      badge = 'Kemungkinan tinggi - disarankan konsultasi klinis';
      tone = 'high';
      classification = inattentionElevated && hyperactivityElevated
        ? 'Pola gejala kombinasi'
        : inattentionElevated
          ? 'Pola gejala terutama tidak perhatian'
          : hyperactivityElevated
            ? 'Pola gejala terutama hiperaktif / impulsif'
            : 'Skrining Bagian A Positif';
    } else if (partAShaded >= 2 || inattentionElevated || hyperactivityElevated) {
      badge = 'Kemungkinan sedang / berkembang';
      tone = 'moderate';
      classification = 'Pola gejala berkembang atau campuran';
    }
  ...
  function scheduleCognitiveTrial() {
    ...
    $('cognitive-target-label').textContent = 'Tunggu…';
  ...
  function domainLevel(shaded, total) {
    ...
    if (ratio >= 0.67) return 'Sangat meningkat';
    if (ratio >= 0.45) return 'Meningkat';
    if (ratio >= 0.22) return 'Sedikit meningkat';
    return 'Dukungan gejala rendah';
  }
  ...
  function displayResults() {
    ...
    $('result-narrative').textContent = results.partAPositive
      ? 'Respons Bagian A Anda mencapai ambang skrining klinis WHO ASRS. Pola ini mengindikasikan perlunya konsultasi terstruktur dengan dokter spesialis kejiwaan (psikiater) atau psikolog klinis; hasil ini bukan diagnosis medis.'
      : results.tone === 'moderate'
        ? 'Respons Anda menunjukkan beberapa gejala yang meningkat tetapi belum mencapai ambang batas Bagian A. Gejala tetap penting untuk diperhatikan jika mengganggu produktivitas atau relasi harian.'
        : 'Respons Anda berada di bawah ambang batas Bagian A. Hasil rendah ini tidak menyingkirkan penyebab medis atau psikologis lain dari kesulitan konsentrasi atau kelelahan mental.';

    $('part-a-score').textContent = `${results.partAShaded} / 6`;
    $('part-a-status').textContent = results.partAPositive ? 'Ambang klinis tercapai' : 'Di bawah ambang';
    ...
    $('report-date').textContent = new Intl.DateTimeFormat('id-ID', { dateStyle: 'medium' }).format(new Date());
    $('report-session').textContent = state.sessionId;
    $('print-part-a').textContent = `${results.partAShaded}/6 respons yang ditandai - ${results.partAPositive ? 'ambang tercapai' : 'di bawah ambang'}`;
    $('print-inattention').textContent = `${results.inattentionRaw}/36 (${results.inattentionShaded}/9 ditandai)`;
    $('print-hyperactivity').textContent = `${results.hyperactivityRaw}/36 (${results.hyperactivityShaded}/9 ditandai)`;
    $('print-cognitive').textContent = cognitive
      ? `Rata-rata WT ${cognitive.meanRT} ms · Variasi WT ${cognitive.rtv} ms · kesalahan komisi ${cognitive.commissionErrors}/5 · omisi ${cognitive.omissionErrors}/15`
      : 'Dilewati';
    $('print-answer-body').innerHTML = QUESTIONS.map((q) => {
      const value = Number.isFinite(state.answers[q.id]) ? state.answers[q.id] : 0;
      return `<tr><td>${q.id}</td><td>${q.part}</td><td>${q.domain === 'inattentive' ? 'Inatensi (Tidak Perhatian)' : 'Hiperaktivitas / Impulsivitas'}</td><td>${OPTIONS[value].label}</td></tr>`;
    }).join('');
  }

  function renderMatrix(results) {
    const axes = [
      ['Penyelesaian Tugas', (results.inattentionShaded / 9) * 100],
      ['Pengorganisasian', ((results.inattentionRaw / 36) + (results.inattentionShaded / 9)) * 50],
      ['Ketahanan Atensi', (results.inattentionRaw / 36) * 100],
      ['Kegelisahan Motorik', (results.hyperactivityShaded / 9) * 100],
      ['Inhibisi Percakapan', (results.hyperactivityRaw / 36) * 100],
      ['Kesabaran Antre / Giliran', ((results.hyperactivityShaded / 9) + (results.hyperactivityRaw / 36)) * 50]
    ];
  ...
  async function shareResult() {
    const results = calculateScores();
    const text = `Saya telah menyelesaikan Skrining ADHD Dewasa FreeIQExam. Bagian A: ${results.partAShaded}/6. Hasil: ${results.badge}. Ini adalah informasi skrining, bukan diagnosis.`;

    try {
      if (navigator.share) {
        await navigator.share({ title: 'Skrining ADHD FreeIQExam', text, url: window.location.href });
        return;
      }
      if (navigator.clipboard && window.isSecureContext) {
        await navigator.clipboard.writeText(text);
        showToast('Ringkasan skrining berhasil disalin.');
        return;
      }
      ...
      if (error?.name !== 'AbortError') showToast('Fitur berbagi tidak tersedia pada peramban ini.');
    }
  }
