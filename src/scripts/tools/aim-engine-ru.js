...
(function () {
  ...
  class AimEngine {
    ...
    getEsportsRank(score, tps, acc) {
      if (score >= 80000 || (tps >= 3.5 && acc >= 95)) {
        return { tier: 'Radiant / Global Elite', percentile: 'Топ 0,5%', badge: 'bg-rose-500 text-zinc-950', desc: 'Моторный рефлекс мирового уровня. Саккадические флики менее 180 мс и пиксельно точное управление прицелом.' };
      }
      if (score >= 65000 || (tps >= 3.0 && acc >= 90)) {
        return { tier: 'Immortal / Faceit Lvl 10', percentile: 'Топ 3%', badge: 'bg-rose-500/20 text-rose-400 border border-rose-500/40', desc: 'Превосходный соревновательный эйм. Очень быстрое поражение целей и надёжные микро-коррекции.' };
      }
      if (score >= 48000 || (tps >= 2.4 && acc >= 85)) {
        return { tier: 'Уровень Diamond', percentile: 'Топ 15%', badge: 'bg-indigo-500/20 text-indigo-400 border border-indigo-500/40', desc: 'Острый тактический рефлекс. Уверенное механическое мастерство с стабильным ритмом фликов.' };
      }
      if (score >= 32000 || (tps >= 1.8 && acc >= 80)) {
        return { tier: 'Уровень Platinum', percentile: 'Топ 35%', badge: 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/40', desc: 'Управление мышью выше среднего. Хорошая базовая координация руки и глаза.' };
      }
      if (score >= 20000) {
        return { tier: 'Уровень Gold', percentile: '50-й перцентиль', badge: 'bg-amber-500/20 text-amber-400 border border-amber-500/40', desc: 'Стандартный любительский эйм. Сосредоточьтесь на постановке руки/кисти и снижении чувствительности для стабильности.' };
      }
      return { tier: 'Silver / Bronze', percentile: 'Развивающийся эйм', badge: 'bg-zinc-500/20 text-zinc-300 border border-zinc-500/40', desc: 'Базовый рефлекс. Тренируйтесь по 10 минут ежедневно с плавным ведением и расстановкой прицела.' };
    }
  ...
  function downloadAimCertificate(results) {
    ...
    const stats = [
      ...
      { label: results.mode === 'tracking' ? 'ВРЕМЯ НА ЦЕЛИ %' : 'ЦЕЛИ / СЕК', value: results.tps.toString() },
      { label: 'СР. РЕАКЦИЯ', value: typeof results.avgReaction === 'number' ? `${results.avgReaction}ms` : results.avgReaction },
      { label: results.mode === 'tracking' ? 'ВРЕМЯ НА ЦЕЛИ' : 'ЦЕЛЕЙ ПОРАЖЕНО', value: results.hits.toString() },
      { label: 'МАКС. СЕРИЯ', value: results.maxStreak.toString().endsWith('s') ? results.maxStreak.toString() : `${results.maxStreak}x` }
    ];
    ...
    ctx.textAlign = 'center';
    ctx.fillText('Verified at ' + resolveShareUrl('https://freeiqexam.com/ru/aim-trainer') + ' · Issued: ' + new Date().toLocaleDateString(), 600, 580);
