...
(function () {
  ...
  class AimEngine {
    ...
    update(dt) {
      ...
      if (this.mode === 'tracking' && this.targets.length > 0) {
        ...
        this.totalTrackingTimeMs += dt * 16.66;
    ...
    drawCrosshair(ctx, x, y) {
      ...
      if (this.crosshairStyle === 'dot') {
        ...
        ctx.fill();
      } else if (this.crosshairStyle === 'cross') {
        const gap = 4;
        const len = 9;
        ctx.beginPath();
        // Top
    ...
    getEsportsRank(score, tps, acc) {
      if (score >= 80000 || (tps >= 3.5 && acc >= 95)) {
        return { tier: 'Radiant / Global Elite', percentile: 'Top 0.5%', badge: 'bg-rose-500 text-zinc-950', desc: 'World-class esports motor reflex. Sub-180ms saccadic flicking and pixel-precise crosshair control.' };
      }
      if (score >= 65000 || (tps >= 3.0 && acc >= 90)) {
        return { tier: 'Immortal / Faceit Lvl 10', percentile: 'Top 3%', badge: 'bg-rose-500/20 text-rose-400 border border-rose-500/40', desc: 'Superior competitive aim. Highly lethal target acquisition and reliable micro-adjustments.' };
      }
      if (score >= 48000 || (tps >= 2.4 && acc >= 85)) {
        return { tier: 'Diamond Tier', percentile: 'Top 15%', badge: 'bg-indigo-500/20 text-indigo-400 border border-indigo-500/40', desc: 'Sharp tactical reflex. Solid mechanical proficiency with consistent flicking rhythm.' };
      }
      if (score >= 32000 || (tps >= 1.8 && acc >= 80)) {
        return { tier: 'Platinum Tier', percentile: 'Top 35%', badge: 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/40', desc: 'Above-average mouse control. Good foundational hand-eye coordination.' };
      }
      if (score >= 20000) {
        return { tier: 'Gold Tier', percentile: '50th Percentile', badge: 'bg-amber-500/20 text-amber-400 border border-amber-500/40', desc: 'Standard recreational aim. Focus on arm/wrist posture and lowering sensitivity for consistency.' };
      }
      return { tier: 'Silver / Bronze', percentile: 'Developing Aim', badge: 'bg-zinc-500/20 text-zinc-300 border border-zinc-500/40', desc: 'Foundational reflex. Practice 10 minutes daily with smooth tracking and crosshair placement.' };
    }
  ...
  function downloadAimCertificate(results) {
    ...
    ctx.textAlign = 'center';
    ctx.fillText(`Verified at https://freeiqexam.com/aim-trainer · Issued: ${new Date().toLocaleDateString()}`, 600, 580);
