...
(function () {
  ...
  class CalorieEngine {
      ...
      switch (this.goal) {
        case 'mild-cut':
          return {
            ...
            desc: 'Mild Deficit (-10%) · Sustainable fat loss with peak strength retention'
          };
        case 'standard-cut':
          return {
            ...
            desc: 'Standard Deficit (-20%) · Gold standard clinical fat loss (1 lb / week)'
          };
        case 'aggressive-cut':
          return {
            ...
            desc: 'Aggressive Deficit (-25%) · Rapid cut with high protein mandate'
          };
        case 'lean-bulk':
          return {
            ...
            delta: Math.round(tdee * 0.10),
            weeklyWeightChangeKg: 0.25,
            desc: 'Lean Surplus (+10%) · Clean hypertrophy with minimal adipose gain'
          };
        case 'heavy-bulk':
          return {
            ...
            delta: Math.round(tdee * 0.20),
            weeklyWeightChangeKg: 0.50,
            desc: 'Aggressive Surplus (+20%) · Maximum strength and muscle gain'
          };
        case 'maintenance':
        default:
          return {
            ...
            desc: 'Maintenance (0 kcal) · Metabolic equilibrium and body recomposition'
          };
      }
    }
    ...
    getNeuroMetabolicMetrics() {
      ...
      return {
        ...
        desc: 'Мозг человека составляет ~2% массы тела, но потребляет ~20% энергии покоя (непрерывная перфузия глюкозы).'
      };
    }
  ...
  const CalorieApp = {
      ...
      if (calcNavyBtn) {
        calcNavyBtn.addEventListener('click', () => {
          ...
          if (bf) {
            ...
            if (bfBadge) {
              bfBadge.textContent = `${bf}% жира в организме учтено`;
    ...
    calculate(animateChart = true) {
      ...
      this.updateElement('resDeltaVal', target.delta > 0 ? `+${target.delta} ккал` : `${target.delta} ккал`);

      // BMR Formula Comparison Box
      this.updateElement('bmrMifflinVal', `${bmrMifflin.toLocaleString()} ккал`);
      this.updateElement('bmrKatchVal', `${bmrKatch.toLocaleString()} ккал`);
      this.updateElement('bmrOxfordVal', `${bmrOxford.toLocaleString()} ккал`);
      ...
      this.updateElement('macroProteinGrams', `${macros.protein.grams} г`);
      this.updateElement('macroProteinKcal', `${macros.protein.calories} ккал (${macros.protein.percent}%)`);

      this.updateElement('macroFatGrams', `${macros.fat.grams} г`);
      this.updateElement('macroFatKcal', `${macros.fat.calories} ккал (${macros.fat.percent}%)`);

      this.updateElement('macroCarbGrams', `${macros.carbs.grams} г`);
      this.updateElement('macroCarbKcal', `${macros.carbs.calories} ккал (${macros.carbs.percent}%)`);

      // Neuro-Metabolic Brain Stats
      this.updateElement('neuroBrainKcal', `${neuro.brainKcal} ккал/день`);
      this.updateElement('neuroBrainGlucose', `${neuro.brainGlucoseGrams} г глюкозы`);
      this.updateElement('neuroBrainWatts', `${neuro.brainHourlyWatts} Вт`);
