(() => {
  ...
  const EYE_SEQUENCE = [
    { id: 'right', label: 'Mata kanan', instruction: 'Cover your LEFT eye with your palm. Do not press on the eyelid.' },
    { id: 'left', label: 'Mata kiri', instruction: 'Cover your RIGHT eye with your palm. Do not press on the eyelid.' },
    { id: 'both', label: 'Kedua mata', instruction: 'Lower your hand and keep both eyes open.' }
  ];
  ...
  function tumblingESvg(sizePx, direction) {
    ...
    return `<svg width="${sizePx}" height="${sizePx}" viewBox="0 0 5 5" shape-rendering="geometricPrecision" role="img" aria-label="Optotipe Tumbling E">
  ...
  function renderResolutionWarning(ppm) {
    ...
    if (sharpest === null) {
      warning.textContent = 'Kepadatan layar terlalu rendah untuk menampilkan optotipe 20/200 pada jarak ini. Mendekatlah atau pilih jarak yang lebih dekat.';
  ...
  function acuityResultFor(eyeId) {
    ...
    if (!eyeState) {
      return { label: 'Belum diuji', measured: false, beyondResolution: false, belowChart: false, untested: true };
    }
    ...
    if (eyeState.beyondResolution) {
      return { label: 'Tidak dapat diukur pada jarak ini', measured: false, beyondResolution: true, belowChart: false, untested: false };
    }
    ...
    return { label: 'Di bawah level 20/200', measured: false, beyondResolution: false, belowChart: true, untested: false };
  }
  ...
  function renderDial() {
    ...
    let svg = `<svg viewBox="0 0 400 400" class="dial-svg" role="group" aria-label="Dial jam astigmatisme dengan dua belas garis radial">`;
    ...
    for (let hour = 1; hour <= 12; hour += 1) {
      ...
      svg += `<g class="dial-ray" data-hour="${hour}" tabindex="0" role="button" aria-pressed="false" aria-label="Posisi jam ${hour}">
  ...
  function updateDialSelection() {
    ...
    if (state.astigmatism.noneReported) {
      label.textContent = 'Anda melaporkan bahwa semua garis terlihat sama jernih.';
      return;
    }
    if (!selected.length) {
      label.textContent = 'Belum ada garis yang dipilih.';
      return;
    }
  ...
  function startDuochrome() {
    ...
    const target = `<svg width="${sizePx}" height="${sizePx}" viewBox="0 0 100 100" role="img" aria-label="Cincin target duokrom">
  ...
  function displayResults() {
    ...
    const noteFor = (result) => {
      if (result.untested) return 'Belum diuji';
  ...
  async function shareResult() {
    ...
    const both = acuityResultFor('both');
    const duo = duochromeInterpretation();
    const text = `I completed the FreeIQExam vision screener. Mata kanan ${right.label}, left eye ${left.label}, both eyes ${both.label}. Duochrome balance: ${duo.short}. This is screening information, not a diagnosis.`;

    try {
      if (navigator.share) {
        await navigator.share({ title: 'Skrining Penglihatan FreeIQExam', text, url: window.location.href });
        return;
      }
