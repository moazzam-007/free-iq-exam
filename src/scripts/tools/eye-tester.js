(() => {
  ...
  const EYE_SEQUENCE = [
    { id: 'right', label: 'Right eye', instruction: 'Cover your LEFT eye with your palm. Do not press on the eyelid.' },
    { id: 'left', label: 'Left eye', instruction: 'Cover your RIGHT eye with your palm. Do not press on the eyelid.' },
    { id: 'both', label: 'Both eyes', instruction: 'Lower your hand and keep both eyes open.' }
  ];
  ...
  function tumblingESvg(sizePx, direction) {
    ...
    return `<svg width="${sizePx}" height="${sizePx}" viewBox="0 0 5 5" shape-rendering="geometricPrecision" role="img" aria-label="Tumbling E optotype">
  ...
  function renderResolutionWarning(ppm) {
    ...
    if (sharpest === null) {
      warning.textContent = 'Screen density is too low to resolve even a 20/200 optotype at this distance. Move closer or choose a shorter distance.';
  ...
  function acuityResultFor(eyeId) {
    ...
    if (!eyeState) {
      return { label: 'Not tested', measured: false, beyondResolution: false, belowChart: false, untested: true };
    }
    ...
    if (eyeState.beyondResolution) {
      return { label: 'Not measurable at this distance', measured: false, beyondResolution: true, belowChart: false, untested: false };
    }
    ...
    return { label: 'Below the 20/200 level', measured: false, beyondResolution: false, belowChart: true, untested: false };
  }
  ...
  function renderDial() {
    ...
    let svg = `<svg viewBox="0 0 400 400" class="dial-svg" role="group" aria-label="Astigmatism clock dial with twelve radiating lines">`;
    ...
    for (let hour = 1; hour <= 12; hour += 1) {
      ...
      svg += `<g class="dial-ray" data-hour="${hour}" tabindex="0" role="button" aria-pressed="false" aria-label="Clock position ${hour}">
  ...
  function updateDialSelection() {
    ...
    if (state.astigmatism.noneReported) {
      label.textContent = 'You reported that all lines look equally sharp.';
      return;
    }
    if (!selected.length) {
      label.textContent = 'No lines selected yet.';
      return;
    }
  ...
  function startDuochrome() {
    ...
    const target = `<svg width="${sizePx}" height="${sizePx}" viewBox="0 0 100 100" role="img" aria-label="Duochrome target ring">
  ...
  function displayResults() {
    ...
    const noteFor = (result) => {
      if (result.untested) return 'Not tested';
  ...
  async function shareResult() {
    ...
    const both = acuityResultFor('both');
    const duo = duochromeInterpretation();
    const text = `I completed the FreeIQExam vision screener. Right eye ${right.label}, left eye ${left.label}, both eyes ${both.label}. Duochrome balance: ${duo.short}. This is screening information, not a diagnosis.`;

    try {
      if (navigator.share) {
        await navigator.share({ title: 'FreeIQExam Vision Screener', text, url: window.location.href });
        return;
      }
