...
(function () {
  ...
  // DEF-60: Localized share URL resolver. Prefers the page canonical URL
    ...
    freqToEarAge(freq) {
      if (freq >= 19000) {
        return {
          ...
          rating: 'Exceptional',
          score: 100,
          desc: 'Ваш слух на высоких частотах исключителен, как у подростка. Вы легко различаете рингтон «комар» 17.4 кГц. Базальные волосковые клетки улитки в идеальном состоянии.'
        };
      }
      if (freq >= 18000) {
        return {
          ...
          rating: 'Superior',
          score: 95,
          desc: 'Превосходный слух на высоких частотах. Вы находитесь в топовой группе для взрослых, с минимальным износом стереоцилий вдоль базального завитка улитки.'
        };
      }
      if (freq >= 17000) {
        return {
          ...
          rating: 'Very Good',
          score: 88,
          desc: 'Очень хороший диапазон высоких частот. Вы находитесь точно на границе, где частота 17.4 кГц затухает для взрослого населения.'
        };
      }
      if (freq >= 16000) {
        return {
          ...
          rating: 'Good',
          score: 80,
          desc: 'Здоровая стандартная слуховая реакция. Начинается ранний доклинический пресбиакузис, что является нормальной биологической особенностью молодого взрослого возраста.'
        };
      }
      if (freq >= 15000) {
        return {
          ...
          score: 72,
          desc: 'Слух выше среднего для взрослого возраста. Разборчивость речи в тишине и при умеренном шуме остаётся полностью сохранной.'
        };
      }
      if (freq >= 14000) {
        return {
          ...
          rating: 'Average',
          score: 64,
          desc: 'Типичный порог слуха среднего возраста. Лёгкий спад 4 кГц–14 кГц является стандартным из-за естественного накопления окружающего шума.'
        };
      }
      if (freq >= 13000) {
        return {
      ...
      if (freq >= 12000) {
        return {
          ...
          rating: 'Mild Decline',
          score: 48,
          desc: 'Обнаружено лёгкое снижение на высоких частотах. Этот порог стандартен для возраста 50+, так как базальные стереоцилии подвергаются ожидаемому кумулятивному старению.'
        };
      }
      if (freq >= 11000) {
        return {
          ...
          score: 40,
          desc: 'Заметное снижение на высоких частотах. Разборчивость речи на фоне шума может быть слегка нарушена. Рекомендуется ежегодный скрининг.'
        };
      }
      if (freq >= 10000) {
        return {
          ...
          score: 32,
          desc: 'Значительное затухание на высоких частотах. Рекомендуется проверка слуха у аудиолога для базирования разговорных частот.'
        };
      }
      if (freq >= 9000) {
        return {
          ...
          score: 24,
          desc: 'Выраженный пресбиакузис. Частоты выше 9 кГц не слышны. Рекомендуется полная клиническая аудиограмма для сохранения разборчивости речи.'
        };
      }
      if (freq >= 8000) {
        return {
          ...
          score: 18,
          desc: 'Существенное снижение слуха на высоких частотах, заходящее в верхнюю речевую полосу (8 кГц). Рекомендуется консультация лицензированного специалиста по слуху.'
        };
      }
      return {
        age: '75+',
        ...
        score: 10,
        desc: 'Верхний предел ниже 8 кГц. Настоятельно рекомендуем пройти клиническую тональную пороговую аудиометрию лично у ЛОР-врача или аудиолога.'
      };
    }
  }
  ...
  class AudiometryTest {
    ...
    classifyThreshold(dbHL) {
      if (dbHL <= 20) return { label: 'Нормальный слух', class: 'normal', textClass: 'text-emerald-600 dark:text-emerald-400', desc: 'Пороги в пределах стандартного клинического диапазона (0–20 дБ УЗД).' };
      if (dbHL <= 40) return { label: 'Лёгкое снижение слуха', class: 'mild', textClass: 'text-amber-600 dark:text-amber-400', desc: 'Трудно слышать тихую речь, шёпотные согласные или речь в шумных кафе.' };
      if (dbHL <= 70) return { label: 'Умеренное снижение слуха', class: 'moderate', textClass: 'text-amber-600 dark:text-amber-400', desc: 'Трудно слышать разговорную речь без усиления.' };
      return { label: 'Значительное снижение слуха', class: 'severe', textClass: 'text-rose-600 dark:text-rose-400', desc: 'Речь не слышна без специализированных слуховых аппаратов или усиления.' };
    }
  ...
  function downloadEarAgeCertificate(data) {
    ...
    // Stats Grid
    const stats = [
      { label: 'Наивысшая частота', value: data.maxFreq + ' Hz' },
      { label: 'Слуховой балл', value: data.score + ' / 100' },
      { label: 'Диапазон возрастной группы', value: data.range }
    ];
    ...
    ctx.fillText('Verified on ' + resolveShareUrl('https://freeiqexam.com/ru/hearing-test') + ' · ' + new Date().toLocaleDateString(), 450, 960);
