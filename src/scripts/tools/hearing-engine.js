...
(function () {
  ...
  class AudioEngine {
    ...
    dbToGain(dbHL) {
      ...
      return clampedGain * this.calibrationMultiplier;
    }
    ...
    freqToEarAge(freq) {
      if (freq >= 19000) {
        return {
          ...
          rating: 'Exceptional',
          score: 100,
          desc: 'Your high-frequency hearing is exceptional, equivalent to a teenager. You can effortlessly detect the 17.4 kHz mosquito ringtone. Your basal cochlear hair cells are in pristine condition.'
        };
      }
      if (freq >= 18000) {
        return {
          ...
          rating: 'Superior',
          score: 95,
          desc: 'Superior high-frequency hearing. You are in the top tier for adults, with minimal stereocilia wear across the basal turn of the cochlea.'
        };
      }
      if (freq >= 17000) {
        return {
          ...
          rating: 'Very Good',
          score: 88,
          desc: 'Very good high-frequency range. You sit right at the boundary where the 17.4 kHz frequency fades for the general adult population.'
        };
      }
      if (freq >= 16000) {
        return {
          ...
          rating: 'Good',
          score: 80,
          desc: 'Healthy, standard auditory response. Early sub-clinical presbycusis is beginning, which is a normal biological hallmark of young adulthood.'
        };
      }
      if (freq >= 15000) {
        return {
          ...
          score: 72,
          desc: 'Above-average hearing for adulthood. Speech intelligibility in quiet and moderate noise remains fully robust and unaffected.'
        };
      }
      if (freq >= 14000) {
        return {
          ...
          rating: 'Average',
          score: 64,
          desc: 'Typical middle-age hearing threshold. A mild 4 kHz–14 kHz slope is standard due to natural environmental noise accumulation.'
        };
      }
      if (freq >= 13000) {
        return {
      ...
      if (freq >= 12000) {
        return {
          ...
          rating: 'Mild Decline',
          score: 48,
          desc: 'Mild high-frequency decline detected. This cutoff is standard for age 50+, as basal stereocilia experience expected cumulative aging.'
        };
      }
      if (freq >= 11000) {
        return {
          ...
          score: 40,
          desc: 'Noticeable high-frequency drop. Background noise speech discrimination may be slightly impaired. Annual screening is recommended.'
        };
      }
      if (freq >= 10000) {
        return {
          ...
          score: 32,
          desc: 'Significant high-frequency attenuation. Hearing health checkup with an audiologist is recommended to baseline conversational frequencies.'
        };
      }
      if (freq >= 9000) {
        return {
          ...
          score: 24,
          desc: 'Marked presbycusis. Frequencies above 9 kHz are inaudible. A full clinical audiogram is recommended to safeguard speech clarity.'
        };
      }
      if (freq >= 8000) {
        return {
          ...
          score: 18,
          desc: 'Substantial high-frequency hearing loss entering the upper speech band (8 kHz). Consultation with a licensed hearing specialist is advised.'
        };
      }
      return {
        age: '75+',
        ...
        score: 10,
        desc: 'Upper limit is below 8 kHz. We strongly advise a clinical in-person pure-tone audiometry exam with an ear-nose-throat (ENT) physician or audiologist.'
      };
    }
  }
  ...
  function downloadEarAgeCertificate(data) {
    ...
    // Stats Grid
    const stats = [
      { label: 'Highest Frequency', value: data.maxFreq + ' Hz' },
      { label: 'Auditory Score', value: data.score + ' / 100' },
      { label: 'Cohort Range', value: data.range }
    ];
    ...
    ctx.fillText('Verified on https://freeiqexam.com/hearing-test · ' + new Date().toLocaleDateString(), 450, 960);
