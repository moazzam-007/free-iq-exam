...
(function () {
  ...
  const STOIC_QUOTES = [
    {
      author: 'Seneca',
      source: 'De Brevitate Vitae (Tentang Singkatnya Kehidupan)',
      text: 'Bukanlah karena waktu hidup kita singkat, melainkan karena kita banyak membuangnya. Hidup ini cukup panjang, dan porsi yang diberikan kepada kita sungguh cukup untuk mencapai hal-hal tertinggi seandainya semuanya digunakan dengan baik.'
    },
    {
      author: 'Marcus Aurelius',
      source: 'Meditasi, Buku II',
      text: 'Anda dapat meninggalkan kehidupan ini sekarang juga. Biarkanlah hal itu menentukan apa yang Anda lakukan, katakan, dan pikirkan.'
    },
    {
      author: 'Seneca',
      source: 'Surat-surat Stoik, Surat I',
      text: 'Genggam setiap jam dalam hidup. Kerjakan tugas hari ini, maka Anda tidak perlu terlalu bergantung pada tugas besok. Sementara kita menunda-nunda, kehidupan berlalu begitu cepat.'
    },
    {
      author: 'Epictetus',
      source: 'Enchiridion',
      text: 'Ingatlah kematian dan pembuangan, serta segala hal yang tampak mengerikan, setiap hari di depan mata Anda, terutama kematian; maka Anda tidak akan pernah memelihara pikiran hina, atau mengingini sesuatu secara berlebihan.'
    },
    {
      author: 'Marcus Aurelius',
      source: 'Meditasi, Buku IV',
      text: 'Waktu adalah sungai, arus peristiwa yang deras, dilihat sekilas dan sudah terbawa lewat, diikuti yang lain lalu hilang.'
    },
    {
      author: 'Seneca',
      source: 'Surat-surat Stoik, Surat XXVI',
      text: 'Renungkan kematian sebelum merenungkan kehidupan; karena hidup tanpa kesadaran akan kematian adalah mimpi, ilusi, dan khayalan yang memabukkan.'
    },
    {
      author: 'Marcus Aurelius',
      source: 'Meditasi, Buku X',
      text: 'Janganlah bertingkah seolah Anda akan hidup sepuluh ribu tahun. Kematian mengintai Anda. Selama Anda hidup, selama masih dalam kuasa Anda, berbaktilah.'
    },
    {
      author: 'Seneca',
      source: 'De Brevitate Vitae',
      text: 'Anda berlaku seperti makhluk fana dalam segala hal yang Anda takuti, dan seperti makhluk abadi dalam segala hal yang Anda inginkan.'
    },
    {
      author: 'Marcus Aurelius',
      source: 'Meditasi, Buku VII',
      text: 'Anggaplah diri Anda telah mati. Anda sudah menjalani hidup Anda. Sekarang, ambillah sisa waktu tersebut dan jalani dengan benar.'
    },
    {
      author: 'Epictetus',
      source: 'Discourses',
      text: 'Ketika Anda mencium anak atau pasangan Anda, katakan pada diri sendiri bahwa Anda mencium seorang manusia fana; sehingga jika mereka meninggal, Anda tidak akan hancur.'
    }
  ];
  ...
  class MementoEngine {
    constructor() {
      ...
      // Milestones Array
      this.milestones = [
        { age: 0, title: 'Kelahiran', color: '#6366f1' },
        { age: 6, title: 'Sekolah Dasar', color: '#6366f1' },
        { age: 18, title: 'Dewasa & Kemandirian', color: '#06b6d4' },
        { age: 22, title: 'Masuk Kuliah / Karier', color: '#06b6d4' },
        { age: 30, title: 'Tonggak Usia 30', color: '#f59e0b' },
        { age: 50, title: 'Puncak Setengah Abad', color: '#f59e0b' },
        { age: 65, title: 'Pensiun & Kebijaksanaan', color: '#f43f5e' }
      ];
    }
    ...
    getWeekInfo(weekIndex) {
      ...
      let era = 'Pembentukan & Masa Kanak';
      let eraColor = '#6366f1';
      if (weekIndex >= 936 && weekIndex < 1820) {
        era = 'Pendidikan, Pembelajaran & Ambisi';
        eraColor = '#06b6d4';
      } else if (weekIndex >= 1820 && weekIndex < 3120) {
        era = 'Penguasaan, Keluarga & Puncak';
        eraColor = '#f59e0b';
      } else if (weekIndex >= 3120) {
        era = 'Kebijaksanaan Lanjut Usia & Warisan';
        eraColor = '#f43f5e';
      }
  ...
  function downloadLifePoster(engine) {
    ...
    ctx.fillText(`HIDUP ANDA DALAM MINGGU · LAHIR: ${engine.birthDate.toLocaleDateString()}`, 60, 98);

    // Right Header Metrics
    ctx.textAlign = 'right';
    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 18px "Geist Mono", monospace';
    ctx.fillText(`${stats.livedWeeks.toLocaleString()} / ${stats.totalWeeks.toLocaleString()} MINGGU`, 1140, 75);
    ctx.fillStyle = '#2997ff';
    ctx.font = '500 13px "Geist Mono", monospace';
    ctx.fillText(`${stats.percentSpent}% SELESAI · ${stats.remainingWeeks.toLocaleString()} TINGGAL`, 1140, 98);
    ...
    ctx.fillText('“Anda berlaku seperti makhluk fana dalam segala hal yang Anda takuti, dan seperti makhluk abadi dalam segala hal yang Anda inginkan.” - Seneca', 600, 605);

    ctx.fillStyle = '#71717a';
    ctx.font = '13px "Geist Mono", monospace';
    ctx.fillText('Dibuat di ' + resolveShareUrl('https://freeiqexam.com/id/memento-mori') + ' · Issued: ' + new Date().toLocaleDateString(), 600, 628);
  ...
  const MementoApp = {
    ...
    render() {
      ...
      this.updateText('statAgeYears', `${stats.ageYears} thn`);
      this.updateText('statLifeExpectancy', `${stats.lifeExpectancyYears} thn`);
    ...
    renderGrid(stats) {
      ...
      if (!gridContainer.dataset.delegated) {
        ...
        gridContainer.addEventListener('mouseover', (e) => {
          ...
          tooltip.innerHTML = `
            <div class="font-bold text-blue-600 dark:text-blue-400 text-sm">Minggu ${info.weekIndex.toLocaleString()} (Usia ${info.ageAtWeek})</div>
            ...
            ${info.milestone ? `<div class="mt-1 font-semibold text-emerald-600 dark:text-emerald-400 text-[13px]">📌 Tonggak: ${info.milestone}</div>` : ''}
          `;
      ...
      for (let r = 0; r < rows; r++) {
        ...
        for (let c = 0; c < cols; c++) {
          ...
          if (index < stats.livedWeeks) {
            ...
            // Current Active Minggu (pulsing halo)
