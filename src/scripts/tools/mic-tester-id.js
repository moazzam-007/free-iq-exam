...
(function () {
  ...
  const SNR_TIERS = [
    { min: 35, label: 'Sangat Baik', tone: 'good', note: 'Wide dynamic range. Your voice will sit clearly above the room.' },
    { min: 25, label: 'Baik', tone: 'good', note: 'Comfortable margin. Fine for calls, streaming and voice notes.' },
    { min: 15, label: 'Cukup', tone: 'moderate', note: 'Audible hiss or hum underneath speech. A quieter room or a closer mic will help.' },
    { min: -Infinity, label: 'Lemah', tone: 'bad', note: 'Background noise is competing with your voice. Treat the room before the microphone.' }
  ];
  ...
  async function openStream() {
    ...
    announce('Mikrofon aktif. Bicara seperti biasa dan perhatikan meterannya.');
  }
  ...
  async function renderHardware() {
    ...
    const label = track && track.label ? track.label : 'Input tidak diketahui';
  ...
  function renderSnr() {
    ...
    if (state.snrDb === null || floor === null || state.speechPeakDb === null || state.speechPeakDb <= DB_FLOOR + 1) {
      if (badge) {
        badge.textContent = 'Mengukur';
        badge.className = 'result-badge neutral';
      }
      if (note) note.textContent = 'Bicara selama beberapa detik dan rasio akan terhitung ketika jendela berisi ucapan dan keheningan.';
      return;
    }
    ...
    if (state.snrDb < 3) {
      if (badge) {
        badge.textContent = 'Tidak ada ucapan terdeteksi';
        badge.className = 'result-badge neutral';
      }
      if (note) note.textContent = 'Sinyal tidak pernah naik secara berarti di atas kebisingan. Pastikan input yang benar dipilih, lalu berbicaralah langsung ke mikrofon.';
      return;
    }
  ...
  async function measureNoiseFloor() {
    ...
    if (status) status.textContent = 'Tetaplah hening - mengukur ruangan…';
  ...
  function resetNoiseFloor() {
    ...
    state.measuringNoise = false;
    const status = $('noise-status');
    if (status) status.textContent = 'Belum diukur.';
    const bar = $('noise-progress');
    if (bar) bar.style.width = '0%';
    const reset = $('noise-reset');
  ...
  function updateDiagnostics() {
    ...
    if (peakDb > -1 || rmsDb > -6) {
      level = 'Terlalu keras - turunkan gain';
      levelTone = 'bad';
    } else if (rmsDb > -24) {
      level = 'Optimal';
      levelTone = 'good';
      ...
      level = 'Terlalu pelan - tingkatkan gain';
      levelTone = 'moderate';
    }
    ...
    // Kejernihan, judged on where speech actually sits in the mix.
    let clarity = 'Hening';
  ...
  const BANDS = [
    { from: 20, to: 150, label: 'Gerombol', color: '#8b5cf6' },
    { from: 150, to: 500, label: 'Fundamental', color: '#6366f1' },
    { from: 500, to: 2000, label: 'Badan', color: '#38bdf8' },
    { from: 2000, to: 6000, label: 'Kejernihan', color: '#10b981' },
    { from: 6000, to: 16000, label: 'Udara', color: '#a78bfa' }
  ];
  ...
  async function playLoopback() {
    ...
    if (button) button.disabled = true;
    if (status) status.textContent = 'Membaca buffer…';
    ...
    if (!samples || !samples.length) {
      if (status) status.textContent = 'Belum ada yang terekam - bicara sebentar, lalu coba lagi.';
      if (button) button.disabled = false;
      return;
    }
    ...
    node.onended = () => {
      if (status) status.textContent = 'Pemutaran selesai.';
    ...
    announce('Memutar kembali beberapa detik terakhir audio yang terekam.');
  }
  ...
  function clearBuffer() {
    ...
    stopPlayback();
    const status = $('playback-status');
    if (status) status.textContent = 'Buffer dibersihkan.';
  }
  ...
  function wireEvents() {
    ...
    if (stopBtn) {
      stopBtn.addEventListener('click', () => {
        ...
        setTransportState(false);
        drawIdleSpectrum();
        announce('Mikrofon dihentikan. Semua audio yang terekam telah dibuang.');
        const status = $('playback-status');
        if (status) status.textContent = 'Mikrofon dihentikan.';
      });
    }
