...
(function () {
  ...
  const SNR_TIERS = [
    { min: 35, label: 'Excellent', tone: 'good', note: 'Wide dynamic range. Your voice will sit clearly above the room.' },
    { min: 25, label: 'Good', tone: 'good', note: 'Comfortable margin. Fine for calls, streaming and voice notes.' },
    { min: 15, label: 'Fair', tone: 'moderate', note: 'Audible hiss or hum underneath speech. A quieter room or a closer mic will help.' },
    { min: -Infinity, label: 'Poor', tone: 'bad', note: 'Background noise is competing with your voice. Treat the room before the microphone.' }
  ];
  ...
  async function populateDevices() {
    ...
    if (!devices.length) {
      ...
      option.value = '';
      option.textContent = 'No microphone detected';
  ...
  async function openStream() {
    ...
    announce('Microphone is live. Speak normally and watch the meters.');
  }
  ...
  async function renderHardware() {
    ...
    const label = track && track.label ? track.label : 'Unknown input';
  ...
  function renderSnr() {
    ...
    if (state.snrDb === null || floor === null || state.speechPeakDb === null || state.speechPeakDb <= DB_FLOOR + 1) {
      ...
      if (note) note.textContent = 'Speak for a few seconds and the ratio will resolve once there is both speech and silence in the window.';
      return;
    }
    ...
    if (state.snrDb < 3) {
      if (badge) {
        badge.textContent = 'No speech detected';
        badge.className = 'result-badge neutral';
      }
      if (note) note.textContent = 'The signal never rose meaningfully above the noise. Check the correct input is selected, then speak directly into the microphone.';
      return;
    }
  ...
  async function measureNoiseFloor() {
    ...
    if (status) status.textContent = 'Stay quiet — measuring the room…';
  ...
  function resetNoiseFloor() {
    ...
    state.measuringNoise = false;
    const status = $('noise-status');
    if (status) status.textContent = 'Not measured yet.';
    const bar = $('noise-progress');
    if (bar) bar.style.width = '0%';
    const reset = $('noise-reset');
  ...
  function updateDiagnostics() {
    ...
    if (peakDb > -1 || rmsDb > -6) {
      level = 'Too hot — pull the gain down';
      levelTone = 'bad';
    } else if (rmsDb > -24) {
      level = 'Optimal';
      levelTone = 'good';
    ...
    // Clarity, judged on where speech actually sits in the mix.
    let clarity = 'Silent';
  ...
  const BANDS = [
    { from: 20, to: 150, label: 'Rumble', color: '#8b5cf6' },
    { from: 150, to: 500, label: 'Fundamentals', color: '#6366f1' },
    { from: 500, to: 2000, label: 'Body', color: '#38bdf8' },
    { from: 2000, to: 6000, label: 'Clarity', color: '#10b981' },
    { from: 6000, to: 16000, label: 'Air', color: '#a78bfa' }
  ];
  ...
  async function playLoopback() {
    ...
    if (button) button.disabled = true;
    if (status) status.textContent = 'Reading the buffer…';
    ...
    if (!samples || !samples.length) {
      if (status) status.textContent = 'Nothing captured yet — speak for a second, then try again.';
      if (button) button.disabled = false;
      return;
    }
    ...
    announce('Playing back the last few seconds of captured audio.');
  }
  ...
  function clearBuffer() {
    ...
    stopPlayback();
    const status = $('playback-status');
    if (status) status.textContent = 'Buffer cleared.';
  }
  ...
  function wireEvents() {
    ...
    if (stopBtn) {
      stopBtn.addEventListener('click', () => {
        ...
        setTransportState(false);
        drawIdleSpectrum();
        announce('Microphone stopped. All captured audio has been discarded.');
        const status = $('playback-status');
        if (status) status.textContent = 'Microphone stopped.';
      });
    }
