(function () {
  ...
  function showRoundResult(milliseconds) {
    ...
    if (milliseconds < 185) {
      refs.roundFeedback.textContent =
        '⚡ Kecepatan luar biasa. Wilayah refleks tingkat elit.';
    } else if (milliseconds < 220) {
      refs.roundFeedback.textContent =
        '🚀 Kilat cepat. Jauh di depan baseline populasi.';
    } else if (milliseconds < 250) {
      refs.roundFeedback.textContent =
        '🎯 Di atas rata-rata. Reaksi visual yang kuat.';
    } else if (milliseconds <= 280) {
      refs.roundFeedback.textContent =
        '⏱️ Baseline manusia yang solid. Pertahankan pandangan rileks.';
    } else {
      refs.roundFeedback.textContent =
        '🐢 Hasil lebih lambat. Latensi tampilan dan input bisa berpengaruh.';
    }
  }
  ...
  function tierFor(milliseconds) {
    ...
    return {
      label: 'Lag Perangkat Keras / Lebih Lambat',
      classes: [
        'bg-amber-50',
        'dark:bg-amber-950',
        'text-amber-700',
  ...
  async function copyScore() {
    ...
    const message =
      ...
      'Can you beat my reflexes? ' + resolveShareUrl('https://freeiqexam.com/id/tes-waktu-reaksi');
