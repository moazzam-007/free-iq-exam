(function () {
  ...
  function updateSoundButton() {
    ...
    refs.soundButton.setAttribute(
      'aria-label',
      soundEnabled ? 'Отключить звуковые эффекты' : 'Включить звуковые эффекты'
    );
    ...
    if (refs.soundLabel) {
      refs.soundLabel.textContent = soundEnabled ? 'Звук' : 'Выкл.';
    }
  }
  ...
  function showRoundResult(milliseconds) {
    ...
    if (refs.roundIndicator) {
      refs.roundIndicator.textContent =
        `ПРОБА ${validTimes.length} ИЗ ${TOTAL_ROUNDS}`;
    }
    ...
    if (milliseconds < 185) {
      refs.roundFeedback.textContent =
        '⚡ Исключительная скорость. Уровень элитных рефлексов.';
    } else if (milliseconds < 220) {
      refs.roundFeedback.textContent =
        '🚀 Молниеносно. Заметно быстрее популяционной нормы.';
    } else if (milliseconds < 250) {
      refs.roundFeedback.textContent =
        '🎯 Выше среднего. Сильная зрительная реакция.';
    } else if (milliseconds <= 280) {
      refs.roundFeedback.textContent =
        '⏱️ Устойчивая человеческая норма. Держите взгляд расслабленным.';
    } else {
      refs.roundFeedback.textContent =
        '🐢 Медленнее обычного. Задержка дисплея и ввода может иметь значение.';
    }
  }
  ...
  function tierFor(milliseconds) {
    if (milliseconds < 185) {
      return {
        label: 'Кибернетический уровень / про-геймер',
        classes: [
          'bg-purple-50',
          'dark:bg-purple-950',
          'text-purple-700',
    ...
    return {
      label: 'Задержка оборудования / медленнее',
      classes: [
        'bg-amber-50',
        'dark:bg-amber-950',
        'text-amber-700',
  ...
  function showFinalSummary() {
    ...
    if (refs.finalPercentile) {
      refs.finalPercentile.textContent =
        `Быстрее ${100 - percentile}% людей (${percentile}-й процентиль)`;
    }
  ...
  async function copyScore() {
    ...
    const message =
      `⚡ Моё среднее время сенсомоторной реакции - ${average} ms в тесте FreeIQExam. ` +
      `Сможете побить мой результат? https://freeiqexam.com/ru/reaction-time-test`;
    ...
    try {
      ...
      try {
        ...
        showToast(
          'Не удалось скопировать - выделите и поделитесь результатом вручную'
        );
      }
