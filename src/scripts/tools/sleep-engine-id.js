 ...
 *   3. Munich ChronoType assessment  (MCTQ -> MSFsc -> Singa (Lion) / Beruang (Bear) / Serigala (Wolf) /
 *                                     Lumba-lumba (Dolphin) overlay)
 ...
 * ordered its depth axis Terjaga < n1 < n2 < rem < n3, placing REM deeper than
 * N2. REM sits near N1 in arousal threshold ("paradoxical sleep"), so the depth
 * axis here is Terjaga < rem < n1 < n2 < n3.
 *
 ...
 *
 * Lumba-lumba (Dolphin) is NOT an MCTQ construct. It comes from Breus's four-chronotype
...
(function () {
  ...
  const STAGE_META = {
    Terjaga: { label: 'Terjaga',     depth: 0, y: 18,  color: '#fbbf24' },
  ...
  const CYCLE_TIERS = {
    6: {
      key: 'optimal',
      label: 'Tidur Restoratif Optimal',
      short: 'Optimal',
      tone: 'optimal',
    ...
    5: {
      key: 'baseline',
      label: 'Standar Rekomendasi Dewasa',
      short: 'Standar',
      tone: 'baseline',
    ...
    4: {
      key: 'minimum',
      label: 'Batas Minimum Fungsional',
      short: 'Minimum',
      tone: 'minimum',
  ...
  const CHRONOTYPES = {
    lion: {
      ...
      scheduleAdvice: 'Protect a bedtime before 22:30. Evening social ' +
                      'obligations cost you more than they cost a Beruang (Bear) or Serigala (Wolf), ' +
                      'because your melatonin onset is already underway.',
      friction: 'Late-evening events, night shifts, and 24-hour light exposure ' +
                'hit this chronotype hardest - you are being asked to stay Terjaga ' +
                'through your biological night.'
    },
    bear: {
      key: 'bear',
      name: 'Beruang (Bear)',
      alias: 'Kronotipe menengah (mayoritas populasi ~55%)',
      tone: 'intermediate',
    ...
    wolf: {
      ...
      summary: 'Your free-day mid-sleep lands late. Your circadian phase is ' +
               ...
               'when a Singa (Lion) is winding down.',
      peak: '17:00 – 21:00',
      trough: '07:00 – 09:00',
    ...
    dolphin: {
      ...
      friction: 'Important caveat: Lumba-lumba (Dolphin) is not part of the Munich ' +
   ...
   * schedule shown elsewhere on the page. A leading Terjaga segment accounts for
   * sleep-onset latency.
   *
  ...
  function buildNightArchitecture(cycleCount) {
    ...
    const totals = { Terjaga: 0, n1: 0, n2: 0, n3: 0, rem: 0 };
  ...
  function renderHypnogram(architecture, bedMin) {
    ...
    const parts = [];
    const stageKeys = ['Terjaga', 'rem', 'n1', 'n2', 'n3'];
  ...
  function computeMCTQ(input) {
    ...
    if (fragmented) {
      // The overlay keeps the Lumba-lumba (Dolphin) descriptive copy but INHERITS the
      // measured MCTQ timing. Showing a fragmented Serigala (Wolf) the Lumba-lumba (Dolphin)'s 23:30
  ...
  function updateChronotype() {
    ...
    // The Lumba-lumba (Dolphin) overlay must disclose what it is overlaying. Silently
    ...
    if (disclosure) {
      if (result.isOverlay) {
        ...
        setText(disclosure,
          'Lumba-lumba (Dolphin) is not an MCTQ classification. Your questionnaire result ' +
          'was ' + result.mctqType.name + ' (MSFsc ' +
          fromMinutes12(result.msfsc * 60) + '); the Lumba-lumba (Dolphin) overlay was ' +
