...
(function () {
  ...
  class TypingEngine {
    ...
    generateWords(count = 120) {
      const pool = (window.TYPING_WORDS && Array.isArray(window.TYPING_WORDS) && window.TYPING_WORDS.length > 0)
        ? [...window.TYPING_WORDS]
        : [...COMMON_WORDS];
      const result = [];
      let lastWord = '';
    ...
    getPercentile(wpm) {
      if (wpm >= 120) return { percentile: 'Top 0,5%', tier: 'Grandmaster Elit', badge: 'bg-amber-500 text-zinc-950', desc: 'Anda memiliki keterampilan mengetik kompetitif kelas dunia, melampaui 99,5% pengetik manusia.' };
      if (wpm >= 95) return { percentile: 'Top 3%', tier: 'Master Touch Typing', badge: 'bg-amber-500/20 text-amber-500 border border-amber-500/30', desc: 'Koordinasi motorik unggul. Kecepatan mengetik Anda melampaui 97% insinyur perangkat lunak profesional.' };
      if (wpm >= 75) return { percentile: 'Top 12%', tier: 'Profesional Mahir', badge: 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30', desc: 'Kecepatan yang sangat produktif. Kemampuan keyboard yang sangat baik untuk menulis, coding, dan tugas klinis.' };
      if (wpm >= 55) return { percentile: 'Top 35%', tier: 'Di Atas Rata-Rata', badge: 'bg-blue-500/20 text-blue-400 border border-blue-500/30', desc: 'Kecepatan mengetik yang lancar dan nyaman di atas rata-rata populasi kantor global.' };
      if (wpm >= 40) return { percentile: 'Persentil ke-50', tier: 'Pengetik Rata-Rata', badge: 'bg-zinc-500/20 text-zinc-300 border border-zinc-500/30', desc: 'Standar kecepatan mengetik dewasa. Menguasai touch typing home-row dapat membuka 70+ WPM.' };
      return { percentile: '25% Terbawah', tier: 'Pengetik Berkembang', badge: 'bg-rose-500/20 text-rose-400 border border-rose-500/30', desc: 'Kecepatan mengetik tingkat dasar. Berlatihlah touch typing tanpa menatap keyboard.' };
    }
  ...
  function downloadTypingCertificate(results) {
    ...
    const stats = [
      ...
      { label: 'WPM MENTAH', value: results.rawWpm.toString() },
      { label: 'KONSISTENSI', value: `${results.consistency}%` },
      { label: 'KARAKTER / MENIT', value: results.cpm.toString() },
      { label: 'DURASI TES', value: results.mode }
    ];
    ...
    ctx.textAlign = 'center';
    ctx.fillText('Verified at ' + resolveShareUrl('https://freeiqexam.com/id/tes-kecepatan-mengetik') + ' · Issued: ' + new Date().toLocaleDateString(), 600, 580);
